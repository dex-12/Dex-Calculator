...
function calculateFormalAir() {
    ...
    document.getElementById('fa-cip').textContent = '$' + (cif).toFixed(2);
...
function resetInformalAirCalculator() {
    ...
    document.getElementById('ia-lc-value').value = '0';
    ...
    document.getElementById('ia-cip').textContent = '₱0.00';
    ...
    document.getElementById('ia-lc-value-group').style.display = 'none';
    document.getElementById('ia-insurance-manual-group').style.display = 'none';
}
...
function printFormalAirCalculation() {
    const totalChargesText = document.getElementById('fa-total-charges').textContent || '₱0.00';
    const dutiableValueText = document.getElementById('fa-dv').textContent || '₱0.00';

    if (totalChargesText === '₱0.00' && dutiableValueText === '₱0.00') {
        alert('Please click Calculate first before printing the Formal Entry Air receipt.');
        return;
    }
    ...
    const fields = {
        ...
        cifValue: document.getElementById('fa-cip').textContent || '$0.00',
    ...
    const logoSrc = new URL('images/logo only.png', window.location.href).href;
    ...
    printWindow.document.write(`
        ...
        <body>
            <div class="print-container">
                <div class="header">
                    <div class="logo"><img src="${logoSrc}" alt="ACB Logo"></div>
                    <div class="company-info">
                        <h2>Andaya-Cutanda Customs Brokerage</h2>
                        <p>Customs & Freight Forwarding Specialist</p>
                        <p>Telephone: +63 09619043375 | info@andaya-cutanda.com</p>
                ...
                <div class="section">
                    <div class="section-title">Computation Breakdown</div>
                    <div class="row"><span class="label">CIP Value</span><span class="value">${fields.cifValue}</span></div>
                    <div class="row"><span class="label">Dutiable Value</span><span class="value">${fields.dutiableValue}</span></div>
                    <div class="row"><span class="label">Customs Duty</span><span class="value">${fields.customsDuty}</span></div>
                    <div class="row"><span class="label">Bank Charge</span><span class="value">${fields.bankCharge}</span></div>
                    <div class="row"><span class="label">Brokerage Fee</span><span class="value">${fields.brokerage}</span></div>
    ...
    // Wait for content to load before opening the browser print dialog.
    setTimeout(() => {
        printWindow.focus();
        printWindow.print();
    }, 300);
...
function resetFormalAirCalculator() {
    ...
    document.getElementById('fa-cip').textContent = '$0.00';
